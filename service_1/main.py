...
async def kafka_worker():
    ...
    try:
        async for msg in consumer:
            ...
            _logger.info(
                f"Service_1 received payload: {payload} (correlation_id={correlation_id})"
            )

            # Пример обработки
            result = {"result": f"Processed by service_1 - {payload}"}

            response = {"correlation_id": correlation_id, "data": result}
            await producer.send_and_wait(TOPIC_RESPONSE, json.dumps(response).encode())

            _logger.info(f"Service_1 sent response for correlation_id={correlation_id}")

    except Exception as e:
        _logger.error(f"Kafka error in service_1: {e}")
    finally:
        await consumer.stop()
        await producer.stop()
        is_ready = False
...
async def main():
    ...
    await site.start()
    _logger.info("Service_1 aiohttp server has started on port 8080")
    try:
        await kafka_task
